	...
	Bump(A)
		...
		if(ismob(A))
			...
			if(M.counter)
				...
				Death(usr)
				return
			var/damage = round(src.nin/0.8*1.8)
			if(damage >= 1)
				M.health -= damage
	...
	Bump(A)
		...
		if(ismob(A))
			...
			if(M.counter)
				...
				Death(usr)
				return
			var/damage = round(src.nin/0.8*1.3)
			if(damage >= 1)
				M.health -= damage
	...
	Bump(A)
		...
		if(ismob(A))
			...
			if(M.counter)
				...
				usr.Death(M)
				return
			var/damage = round(src.nin/0.8*1.5)
			if(damage >= 1)
				M.health -= damage
	...
	verb
		Focus()
			...
			if(!usr.focusing&&!usr.firing)
				...
				usr.focusing =1
				usr.shurikenskill = usr.shurikenskill*1.3
				usr.kunaiskill = usr.kunaiskill*1.3
				usr.trapskill = usr.trapskill*1.3
				fire
				if(usr.focusing)
					usr.chakra -=50
					if(usr.chakra <= 4)
